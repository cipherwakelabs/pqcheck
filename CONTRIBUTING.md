...
Thanks for your interest. This repo hosts the `pqcheck` CLI (`/cli`) and the companion GitHub Action (`/action`). Both share the same scoring methodology, documented at [cipherwake.io/methodology](https://www.cipherwake.io/methodology).
...
- **Methodology feedback:** open an issue tagged `methodology`. Cite the relevant page under [`/methodology/`](https://www.cipherwake.io/methodology). Score-weighting changes are version-bumped (v1.x) and documented in the per-tool changelog before they ship.
- **Code:** read the rest of this file.

## Local dev

```bash
git clone https://github.com/cipherwakelabs/pqcheck.git
cd pqcheck/cli
node bin/pqcheck.js stripe.com
```

The CLI is plain Node.js (>=18), zero build step, **zero npm dependencies**. The bin file is self-contained and calls `https://www.cipherwake.io/api/scan` via the built-in `fetch`. Keep it that way — every added dependency becomes a supply-chain surface for a tool whose job is to find supply-chain surfaces.
...
`pqcheck` has no funding, no sponsors, and no commercial backing. Cipherwake is bootstrapped; there are no donation, GitHub Sponsors, or grant channels accepting contributions, financial or otherwise. The hosted scanner at [cipherwake.io](https://www.cipherwake.io) is run as a free public utility, and the CLI / Action / extension are MIT-licensed open source.
